...
SMART_QUESTION_BANK: dict[str, list[ClarificationQuestion]] = {
    "pain_red_flags": [
        ...
        ClarificationQuestion(
            ...
            question=(
                "Do you have tingling, weakness, swelling, or skin that "
                "feels cold or looks blue?"
            ),
            options=[
    ...
    "chest_cardiac": [
        ...
        ClarificationQuestion(
            id="chest_associated",
            question=(
                "Do you also have sweating, nausea, shortness of breath, "
                "or pain spreading to jaw/arm?"
            ),
            options=[
                "Sweating",
                "Nausea",
    ...
    "orthopedics": [
        ...
        ClarificationQuestion(
            id="ortho_deformity",
            question=(
                "Is there visible deformity, major swelling, bruising, "
    ...
    "neurology": [
        ClarificationQuestion(
            id="neuro_deficit",
            question=(
                "Do you have weakness, numbness, one side of the face "
                "drooping, confusion, or trouble speaking?"
            ),
            options=[
                "Weakness",
                "Numbness",
    ...
    "ent": [
        ClarificationQuestion(
            ...
            question=(
                "Are you having trouble swallowing, breathing, hearing, "
                "or severe throat/ear pain?"
            ),
            options=[
                "Trouble swallowing",
                "Trouble breathing",
                "Hearing change",
                "Severe throat/ear pain",
                "None",
            ],
        ),
    ],
    "dermatology": [
        ClarificationQuestion(
            ...
            question=(
                "Is the rash spreading quickly, painful, blistering, or "
...
def _condition_names(summary: object | None) -> str:
    ...
    return " ".join(
        str(getattr(condition, "name", "")) for condition in conditions
    ).lower()
...
def _build_llm_prompt(
    *,
    ...
    return (
        ...
        "Generate questions that best differentiate between the listed possible "
        "conditions and identify urgent red flags. Do not diagnose. Do not give "
        "advice. Do not ask for name, phone, address, national ID, insurance, "
        "or other identifying details.\n\n"
        "Return ONLY valid JSON: an array of 2 to 4 objects with this exact shape:\n"
        '[{"id":"short_snake_case","question":"patient-facing question",'
        '"options":["choice 1","choice 2","choice 3"]}]\n\n'
        "Rules:\n"
        "- Prefer multiple choice options with 3 to 5 choices.\n"
        "- Options must be self-contained; avoid plain 'Yes' or 'No' unless "
        "the option repeats what yes/no means.\n"
        "- Good: 'Yes - numbness or tingling is present'. Bad: 'Yes'.\n"
        "- Each question must target missing information that changes urgency, "
        "likely condition, or specialty.\n"
        "- Ask about onset, location, severity, triggers, spread, function, "
        "associated symptoms, and red flags only when relevant.\n"
        "- Keep wording simple, non-alarming, and understandable to a patient "
        "without medical training.\n"
        "- Prefer everyday language such as 'trouble breathing' over technical "
...
def _generate_llm_questions(
    *,
    ...
    try:
        ...
        if questions:
            logger.info(
                "llm_clarification_questions_generated count=%s", len(questions)
            )
        return questions
