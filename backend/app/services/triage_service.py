...
def triage(
    ...
    if (
        {"back pain", "neck pain"}.intersection(base_features.symptoms)
        and {"numbness", "weakness"}.intersection(base_features.symptoms)
    ):
    ...
    logger.info(
        "triage_completed triage_level=%s query_length=%s contexts=%s specialty=%s condition=%s confidence=%s unsupported_reasoner_high=%s",
        triage_level,
    ...
    recommended_actions = (
        actions
        if unsupported_reasoner_high
        else summary.recommended_actions
    ...
    response_red_flags = (
        []
        if unsupported_reasoner_high
        else summary.red_flags
        if hasattr(summary, "red_flags")
        else []
    )
